...
const SLIDES = [
  {
    key: "docs",
    bullet: "Real-Time Threat Detection",
    accent: "emerald",
    pillText: "Stop",
    tailText: "zero-day exploits",
    body: "Instantly identify anomalies and malicious patterns. Our AI models continuously learn from global attack data.",
    description: "Instantly identify anomalies and malicious patterns across your network to stop zero-day exploits before they breach your defenses.",
    highlights: [
      { word: "anomalies", color: "emerald" },
      { word: "global attack data", color: "violet" },
    ],
    collaborators: [
      { letter: "A", color: "emerald" },
      { letter: "I", color: "violet" },
    ],
  },
  {
    key: "comments",
    bullet: "Blockchain-Backed Audit Logs",
    accent: "amber",
    pillText: "Verify with",
    tailText: "immutable records",
    body: "Maintain an immutable, cryptographically verifiable record of all security events and system changes.",
    description: "Ensure total compliance and transparent incident forensics with our decentralized, immutable blockchain ledger.",
    highlights: [
      { word: "immutable", color: "amber" },
      { word: "cryptographically verifiable", color: "sky" },
    ],
    collaborators: [
  ...
  {
    key: "commands",
    bullet: "Self-Healing Infrastructure",
    accent: "violet",
    pillText: "Automate",
    tailText: "remediation",
    body: "Automatically isolate compromised nodes and seamlessly reroute traffic without requiring manual intervention.",
    description: "CyberDefenseX dynamically patches vulnerabilities and restores healthy states instantly and automatically.",
    highlights: [
      { word: "isolate", color: "violet" },
      { word: "reroute", color: "emerald" },
    ],
    collaborators: [
      { letter: "S", color: "violet" },
      { letter: "H", color: "emerald" },
    ],
  },
];
...
/* ══════════════════════════════════════════════════════════
   Main component
══════════════════════════════════════════════════════════ */
/* ── Section ID map for navbar anchor links ── */
const NAV_LINKS = [
  { label: "Platform", target: "hero" },
  { label: "Features", target: "features" },
  { label: "Pricing", target: "pricing" },
  { label: "About", target: "footer-cta" },
];
...
export default function Landing() {
  ...
  return (
    ...
    >
      ...
      <header className="relative z-10">
        <nav className="flex max-w-7xl md:px-6 mr-auto ml-auto pt-4 pr-4 pb-4 pl-4 items-center justify-between">
          ...
          <div className="hidden items-center gap-8 md:flex">
            ...
            <a href="https://github.com/cyberhub2025/CyberDefenseX" target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-medium text-slate-300 hover:text-white border border-white/10 bg-white/5 transition-all duration-300 hover:border-white/20 hover:bg-white/10">
            ...
            <button onClick={() => nav("/login")} className="text-sm font-medium text-slate-300 hover:text-white bg-transparent border-none cursor-pointer">Log in</button>
            <button onClick={() => nav("/login?mode=register")}
        ...
        {menuOpen && (
          <div className="mx-auto max-w-7xl px-4 md:hidden">
            <div className="space-y-1 rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur">
              {NAV_LINKS.map((l) => (
                <button key={l.label} onClick={() => scrollTo(l.target)} className="block w-full text-left rounded-lg px-3 py-2 text-sm font-medium text-slate-200 hover:bg-white/5 bg-transparent border-none cursor-pointer">{l.label}</button>
              ))}
              <a href="https://deepwiki.com/cyberhub2025/CyberDefenseX_Pro" target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-sky-300 hover:bg-sky-500/10 no-underline"
              ...
              <a href="https://github.com/cyberhub2025/CyberDefenseX_Pro" target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-slate-200 hover:bg-white/5 no-underline">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="text-slate-300">
                  <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                </svg>
                View on GitHub
              </a>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => nav("/login")} className="flex-1 rounded-lg border border-white/10 bg-black/40 px-3 py-2 text-center text-sm font-medium text-slate-200 cursor-pointer">Log in</button>
                <button onClick={() => nav("/login?mode=register")} className="flex-1 rounded-lg bg-white px-3 py-2 text-center text-sm font-medium text-black border-none cursor-pointer">Sign up</button>
              </div>
            </div>
          </div>
        )}
      ...
      <section id="hero" className="relative z-10">
        <div className="mx-auto max-w-7xl px-4 pt-10 pb-8 md:px-6 md:pt-16">
          <div className="mx-auto max-w-3xl text-center">
            <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-300">Autonomous. Transparent. Unbreakable.</p>
            <h1 className="sm:text-5xl md:text-7xl text-4xl font-semibold tracking-tight">Secure your systems at lightspeed</h1>
            ...
            <div className="mt-6 flex items-center justify-center gap-3 text-sm text-slate-400">
              ...
              <span>Securing the infrastructure of modern enterprises worldwide.</span>
            </div>
          </div>
        </div>
                  ...
                  <div className="relative overflow-hidden rounded-xl border border-white/10 bg-white/5 ring-1 ring-white/10">
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                      <div className="max-w-xl rounded-xl border border-white/10 bg-black/40 p-4 backdrop-blur">
                        <h3 className="text-3xl sm:text-4xl font-semibold tracking-tight">CyberDefenseX Threat Command Center</h3>
                        <p className="mt-1 text-sm text-slate-300">A unified, real-time dashboard giving you unprecedented visibility into your network’s security posture, active threats, and automated mitigation workflows.</p>
      ...
      {/* ══════════ FEATURES ══════════ */}
      <div id="features" className="relative overflow-hidden antialiased z-10 text-neutral-200 bg-black"
        ...
        <section className="max-w-7xl mx-auto px-6 md:px-8 py-24 md:py-32">
          <div className="text-center mb-16 md:mb-20">
            <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-medium text-slate-300"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 text-emerald-400"><rect width="8" height="8" x="3" y="3" rx="2" /><path d="M7 11v4a2 2 0 0 0 2 2h4" /><rect width="8" height="8" x="13" y="13" rx="2" /></svg>Next-Generation Security</p>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-semibold tracking-tight text-white mb-4">Everything you need to detect and respond faster</h2>
            <p className="text-lg text-slate-300 max-w-2xl mx-auto">From zero-day detection to instant automated response, CyberDefenseX supercharges your SOC—slashing response times, eliminating human error, and fortifying your perimeter.</p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
            {/* Bullet list */}
            <div className="order-2 lg:order-1">
              <h3 className="text-2xl sm:text-3xl lg:text-4xl font-semibold tracking-tight text-white mb-8">Intelligent workflows that defend your infrastructure</h3>
              ...
              <div className="relative rounded-2xl border border-neutral-800/80 bg-neutral-900/50 shadow-[0_20px_80px_-20px_rgba(0,0,0,0.8)] ring-1 ring-white/5 backdrop-blur-md overflow-hidden">
                ...
                <div className="h-14 flex items-center gap-3 px-6 border-b border-neutral-800/70 text-neutral-400">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    <span className="truncate font-medium">CyberDefenseX</span>
                    <span className="opacity-50">›</span>
                    <span className="truncate text-neutral-300 font-medium">Threat Intelligence</span>
                  </div>
      ...
      {/* ══════════ PRICING ══════════ */}
      <div id="pricing" className="py-24 px-6 relative z-10">
        <div className="max-w-7xl mx-auto">
          <div className="border border-white/10 rounded-3xl p-12 lg:p-16"
            ...
            <div className="text-center mb-16">
              ...
              <h3 className="text-3xl lg:text-4xl text-white tracking-tight mb-6 font-semibold">Enterprise-grade security for every team</h3>
              <p className="text-lg text-slate-300 max-w-3xl mx-auto">From rapid-growth startups to global enterprises, CyberDefenseX scales seamlessly. Choose the defense tier that fits your threat landscape.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="relative rounded-2xl border border-white/10 p-8" style={{ background: "rgba(255,255,255,0.02)" }}>
                <div className="mb-8">
                  ...
                  <p className="text-sm text-slate-400 mb-6">Essential protection for individuals, researchers, and small teams.</p>
                ...
                <ul className="space-y-4 mb-8">{["3 Monitored Assets", "Core AI Detection", "Community Threat Intelligence", "Basic Email Alerts", "Community Support"].map((f) => <li key={f} className="flex items-center gap-3"><Check /><span className="text-sm text-slate-300">{f}</span></li>)}</ul>
                <button onClick={() => nav("/login?mode=register")} className="w-full py-4 px-6 rounded-xl text-sm font-medium text-white border border-white/20 hover:border-white/30 transition-all duration-300 hover:bg-white/5" style={{ background: "rgba(255,255,255,0.03)" }}>Get Started Free</button>
              </div>
              ...
              <div className="relative rounded-2xl border border-sky-500/30 p-8" style={{ background: "rgba(14,165,233,0.05)" }}>
                ...
                <div className="mb-8">
                  ...
                  <p className="text-sm text-slate-400 mb-6">Advanced defense features for dedicated security teams.</p>
                  <div className="flex items-baseline gap-2"><span className="text-4xl text-white font-semibold">$49</span><span className="text-sm text-slate-400">per month</span></div>
                </div>
                <ul className="space-y-4 mb-8">{["Unlimited Assets", "Automated Mitigation", "Real-Time Integrations", "Blockchain Audit Logs", "Priority Support"].map((f) => <li key={f} className="flex items-center gap-3"><Check /><span className="text-sm text-slate-300">{f}</span></li>)}</ul>
              ...
              <div className="relative rounded-2xl border border-white/10 p-8" style={{ background: "rgba(255,255,255,0.02)" }}>
                <div className="mb-8">
                  ...
                  <p className="text-sm text-slate-400 mb-6">Uncompromising security for large organizations with complex needs.</p>
                ...
                <ul className="space-y-4 mb-8">{["Custom AI Training", "Dedicated Node Clusters", "On-Premise Deployment Options", "White-glove Onboarding", "24/7 Dedicated Analyst Support"].map((f) => <li key={f} className="flex items-center gap-3"><Check /><span className="text-sm text-slate-300">{f}</span></li>)}</ul>
            ...
            <div className="text-center mt-16 pt-12 border-t border-white/10">
              <p className="text-sm text-slate-400 mb-6">All plans include robust encryption, 99.99% uptime guarantee, and global CDN coverage. No hidden fees.</p>
      ...
      {/* ══════════ FOOTER ══════════ */}
      <footer id="footer-cta" className="relative z-10">
        <div className="mx-auto max-w-7xl px-4 py-12 md:px-6">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 text-center">
            <div className="mx-auto max-w-2xl">
              <h2 className="text-2xl md:text-3xl font-semibold tracking-tight">Defend autonomously. Scale confidently.</h2>
              <p className="mt-2 text-sm text-slate-400">Leave manual remediation behind. Let CyberDefenseX secure your infrastructure so you can focus on building the future.</p>
              <div className="mt-4 flex items-center justify-center gap-3">
                <a href="#" onClick={(e) => { e.preventDefault(); nav("/login?mode=register"); }} className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-xs font-medium text-black hover:bg-white/90">
          ...
          <div className="mt-6 flex flex-col items-center justify-between gap-3 text-xs text-slate-500 sm:flex-row">
            <span>© {new Date().getFullYear()} CyberDefenseX Technologies</span>
